...
class App extends Component {
  ...
  selectedCatalog(index) {
    this.setState({
      ...
      catalogSelected: this.state.catalogs[parseInt(index)]
    });
  }
  ...
  previousClick() {
    if (this.state.currentIndex !== 0) {
      ...
      let newOrder = [...this.state.catalogs];
      let temp = newOrder.pop();
      newOrder.unshift(temp);
      this.setState({ catalogs: newOrder }, () => this.selectedCatalog(0));
    }
  }
  ...
  nextClick() {
    if (this.state.currentIndex !== this.state.catalogs.length - 1) {
      ...
      let newOrder = [...this.state.catalogs];
      let temp = newOrder.shift();
      newOrder.push(temp);
      this.setState({ catalogs: newOrder }, () =>
        this.selectedCatalog(this.state.catalogs.length - 1)
      );
    }
  }
  ...
  resetSlideTimer(isActive = false) {
    isActive && this.startSlide()
    !isActive && this.stopSlide()
  }
